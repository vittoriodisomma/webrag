...
class VDB:
    ...
    def insert(self, url, text):
        ...
        if result: 
            ...
            try:
                ...
                out = client.insert(self.collection, entity)
                if out:
                   result = 'Caricamento effettuato su Milvus'
            except Exception as e:
                result = f'Errore di caricamento: {e}'
            return result        
    ...
    def search(self, text):
        ...
        results = client.search(collection_name=collection, data=data, anns_field=anns_field,
                                search_param=params, limit= 3, output_fields=output_fields)[0]
        search_result = []
        for item in results:
